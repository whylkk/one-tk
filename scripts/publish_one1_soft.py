...
# Keep.js-compatible header (Shawn resource parser)
HEADER = r'''/*
 *
 *
...
def main():
    ...
    # Ensure body does not start with another block comment that confuses parser
    out = HEADER + body
    Path("one1.js").write_text(out, encoding="utf-8")
    print("wrote one1.js", len(out))

    # Pure conf for users who enable resource parser (recommended)
    conf = r'''#!name=One1
