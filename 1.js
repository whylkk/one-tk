...
/**
 * One · 圈叉脚本（已验证：box AES+zlib → token_one → detail 真链）
 *
 * 根因修复：
 * box/api/config 返回 application/octet-stream：
 *   AES-128-CBC 解密 → zlib inflate → JSON（含 token_one）
 * 免费用户 App 响应里的 video 是假链；必须用 token_one 重拉 detail。
 *
...
const SCRIPT_VERSION = 'BUY_REAL_RETURN_LIST_20260828_prefs_ignoreExpire';
const DEBUG = true;
const STORE_KEY = 'one_core_token_v3';

// ==================== 开关 ====================
// true  = 忽略 JWT 过期（当前 box/GitHub 都返回过期 token 时用）
// false = 恢复正常过期检查（上游修好后改回 false 即可）
...
const BOX_DOMAINS = [
  ...
  'http://202.95.22.200:9672/',
  'http://202.95.22.201:9672/',
  'http://202.95.22.202:9672/',
  'http://198.44.248.101:9672/',
  'http://198.44.248.102:9672/',
  'http://122.10.20.249:9672/',
];
const DEFAULT_MEDIA = {
  one: '',
...
/**
 * QX $task.fetch 会把 box 二进制 body 弄坏（UTF-8 破坏、长度不对齐），
 * AES+zlib 解析失败。这里内置已验证可用的 token_one（与 core 同一套）。
 * exp ≈ 2026-08-20；过期后需重新下发或换 FALLBACK。
 */
const TOKEN_JSON_URL = 'https://raw.githubusercontent.com/whylkk/one-tk/refs/heads/main/token.json';

const FALLBACK_TOKEN_ONE = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..SMiLO7x3FMdCOF6hoE6ysqXQngl7trteAOHob8SGNmA';
const FALLBACK_SESSION = {
  tokenOne: FALLBACK_TOKEN_ONE,
  baseUrl: 'https://api.einhn4.com/',
  media: {
    one: 'https://api.einhn4.com/',
    one_img: 'https://jmt612.xqjby.com/',
    one_video: 'https://dlmk0129.scycjz.com/',
  },
  tokenAt: 0,
...
function zlibInflateBytes(bytes) {
  ...
  // tiny-inflate returns subarray or writes to out
  if (result && result.length) return result;
  // find actual length by scanning null? better: tinf returns sliced
  return out;
}
...
function storeWrite(obj){
  try{
    ...
    // Quantumult X：官方持久化是 $prefs（不是 $persistentStore）
...
/** 距过期不足 TOKEN_SKEW_SEC（默认 1 小时）视为需要刷新 */
const TOKEN_SKEW_SEC = 3600;
function tokenExpiringSoon(token){
  // 开关：IGNORE_TOKEN_EXPIRE = true 时永远返回 false（不视为过期）
...
/** box body: latin1/binary string → AES → zlib → JSON */
function parseBoxBody(text){
  text=String(text||'');
  if(!text)return null;
  const t=text.trim();
  if(t.charAt(0)==='{'){try{return JSON.parse(t);}catch(_){}}
  // 1) binary/latin1 bytes → AES → zlib
  try{
    const bytes=[];
    for(let i=0;i<text.length;i++)bytes.push(text.charCodeAt(i)&0xff);
    const n=bytes.length-(bytes.length%16);
    if(n>=16){
      const raw=aesDecryptBytes(bytes.slice(0,n),BOX_AES_KEY,BOX_AES_IV);
      if(raw[0]===0x78){
        const inflated=zlibInflateBytes(raw);
        let str=utf8Decode(Array.from(inflated));
        const z=str.indexOf('\0'); if(z>=0)str=str.slice(0,z);
        const last=str.lastIndexOf('}');
        if(last>0)return JSON.parse(str.slice(0,last+1));
      } else {
        const str=utf8Decode(raw).trim();
        if(str.charAt(0)==='{')return JSON.parse(str);
      }
    }
  }catch(e){step('box.bin',e.message||e);}
  // 2) base64 AES
  try{
    const c=t.replace(/^"|"$/g,'').replace(/\s+/g,'');
    if(c.length>=16&&/^[A-Za-z0-9+/=]+$/.test(c)){
      const raw=aesDecryptBytes(b64Decode(c),BOX_AES_KEY,BOX_AES_IV);
      if(raw[0]===0x78){
        const inflated=zlibInflateBytes(raw);
        let str=utf8Decode(Array.from(inflated));
        const last=str.lastIndexOf('}');
  ...
  }catch(e){step('box.b64',e.message||e);}
  return null;
}

function applyBoxJson(json){
  if(!json||(json.code!=0&&json.code!=200&&json.code!='0'&&json.code!='200'))return null;
  const data=json.data||{};
  const media={one:'',one_img:DEFAULT_MEDIA.one_img,one_video:DEFAULT_MEDIA.one_video};
  const apis=data.api||[];
  for(let i=0;i<apis.length;i++){
    const it=apis[i];if(!it||!it.name||!it.host)continue;
    const host=it.host.endsWith('/')?it.host:it.host+'/';
    if(it.name==='one')media.one=host;
    if(it.name==='one_img')media.one_img=host;
    if(it.name==='one_video')media.one_video=host;
  }
  const tokens=data.token||[];
  let tokenOne='';
  for(let i=0;i<tokens.length;i++){if(tokens[i]&&tokens[i].name==='token_one'){tokenOne=tokens[i].token;break;}}
  if(!tokenOne)return null;
  let baseUrl=media.one||ONE_DOMAINS[0];
  if(!/^https:\/\//i.test(baseUrl))baseUrl=ONE_DOMAINS[0];
  const saved={tokenOne:String(tokenOne),baseUrl:String(baseUrl),media:media,tokenAt:nowSec()};
  storeWrite(saved);
  step('box.ok','tokenLen='+tokenOne.length+' base='+baseUrl+' videoHost='+media.one_video);
  return saved;
}

function refreshTokenFromBox(cb){
  if(typeof $task==='undefined'||!$task.fetch){step('box.noTask');cb(null);return;}
  const hdr={'Content-Type':'application/x-www-form-urlencoded',Accept:'*/*','User-Agent':'Dart/3.0 (dart:io)'};
  const body='channel='+CHANNEL;
  let idx=0,lastErr='';
  function next(){
    if(idx>=BOX_DOMAINS.length){step('box.fail',lastErr);cb(null);return;}
    const base=BOX_DOMAINS[idx++];
    const url=base.replace(/\/+$/,'')+'/box/api/config';
    step('box.try',url);
    $task.fetch({url:url,method:'POST',headers:hdr,body:body}).then(resp=>{
      try{
        const text=resp&&resp.body!=null?String(resp.body):'';
        step('box.raw','st='+(resp.statusCode||'')+' len='+text.length+' magic='+(text.length?text.charCodeAt(0).toString(16):''));
        const json=parseBoxBody(text);
        if(!json){lastErr='parse len='+text.length;next();return;}
        const saved=applyBoxJson(json);
        if(!saved){lastErr='no token code='+json.code;next();return;}
        cb(saved);
      }catch(e){lastErr=String(e.message||e);next();}
    }).catch(e=>{lastErr=String(e);next();});
  }
  next();
}

function ensureSession(cb){
  // ① 持久化缓存：JWT 距过期 > TOKEN_SKEW_SEC → 直接用，不打任何网络
  const cached = storeRead();
  if (cached && cached.tokenOne && !tokenExpiringSoon(cached.tokenOne)) {
    step('session.cache', 'hit left=' + tokenLeftSec(cached.tokenOne) + 's len=' + cached.tokenOne.length + (IGNORE_TOKEN_EXPIRE?' [ignoreExpire]':''));
    cb(cached);
    return;
  }

  // ② 无缓存 / 将过期 / 已过期 → 才请求 GitHub raw
  step('session.refresh', cached ? ('expired left=' + tokenLeftSec(cached.tokenOne) + 's') : 'no_cache');
  fetchTokenFromGithub(function (saved) {
    if (saved && saved.tokenOne && !tokenExpiringSoon(saved.tokenOne)) {
      step('session.github', 'ok left=' + tokenLeftSec(saved.tokenOne) + 's len=' + saved.tokenOne.length + (IGNORE_TOKEN_EXPIRE?' [ignoreExpire]':''));
      storeWrite(saved); // 写入 $persistentStore，下次 detail 直接命中缓存
      cb(saved);
      return;
    }

    // ③ GitHub 失败 → 内置 FALLBACK（仍写入缓存）
    if (FALLBACK_TOKEN_ONE && !tokenExpiringSoon(FALLBACK_TOKEN_ONE)) {
      ...
      step('session.fallback', 'left=' + tokenLeftSec(fb.tokenOne) + 's' + (IGNORE_TOKEN_EXPIRE?' [ignoreExpire]':''));
      storeWrite(fb);
      cb(fb);
      return;
    }
...
function fetchTokenFromGithub(cb){
  ...
  $task.fetch({url:url,method:'GET',headers:{'Accept':'*/*','User-Agent':'Quantumult%20X'}}).then(function(resp){
    try{
      ...
      // 注意：这里不再因为即将过期而直接丢弃，由上层 tokenExpiringSoon 统一决定
      // （IGNORE_TOKEN_EXPIRE=true 时永远不会视为过期）
...
/** 优先真链：含 decrypt / compress/decry 的路径 */
function isFakeVideoPath(p){
  p=String(p||'');
  // 免费用户常见假链：/one/20240326/xxx.mp4 且无 compress/decry、无 decrypt
...
function notify(title,subtitle,body,openUrl){
  ...
  // 不要把 SenPlayer:// 塞进 media-url（QX 会报 Unsupported URL）
...
function markPurchased(node){
  ...
  // 常见购买/解锁字段（0/1 或 true/false 都覆盖）
  const buyKeys=[
...
/** 用 token_one 重拉到的 detail data 组装回包，并标已购买 */
...
function handleDetailAndNotify(passthroughJson,wasEncrypted){
  ...
  ensureSession(session=>{
    if(!session||!session.tokenOne){
      ...
      notify('One','无 token_one','box 下发失败，无法取真链','');
      // 无 token 时仍把原响应标成已购买，避免客户端卡购买页
    ...
    postOne(session,'v2.5/article/detail',{id:id},(data,err)=>{
      ...
      // ★ 关键：把 token_one 拉到的真链详情回给客户端，并标已购买
