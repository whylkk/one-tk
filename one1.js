/**
 * One · 圈叉脚本 one1（bootstrap 续期优先 + GitHub 兜底）
 *
 * 续期顺序：
 *   ① 缓存有效 → 直接用
 ...
 *
 * 脚本地址：
 *   https://raw.githubusercontent.com/whylkk/one-tk/main/one1.js
 *
 * [rewrite_local]
 * ^https?://[^/]+/v2\.5/bootstrap url script-response-body one1.js
 * ^https?://[^/]+/v2\.5/vip/download url script-response-body one1.js
 * ^https?://[^/]+/v2\.5/article/detail url script-response-body one1.js
 * ^https?://[^/]+/v2\.5/article/(day|discovery|search|list) url script-response-body one1.js
 * ^https?://[^/]+/v2\.5/series/(list|chapters) url script-response-body one1.js
